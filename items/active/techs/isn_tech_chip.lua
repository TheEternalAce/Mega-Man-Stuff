...
function init()
  ...
  self.tech_to_unlock = config.getParameter("tech_to_unlock")
  self.unlock_message = config.getParameter("unlock_message")
...
function update(dt, fireMode, shiftHeld)
  ...
  if storage.firing and animator.animationState("firing") == "off" then
    if player then
      player.makeTechAvailable(self.tech_to_unlock)
      item.consume(1)
    end
